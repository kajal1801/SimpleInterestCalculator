...
function updateRateByInput() {
    ...
    document.getElementById('rate_val').value = Math.min(Math.max(1, Math.round(rate_val*4)/4), 100);
    document.getElementById('rate').value = Math.min(Math.max(1, Math.round(rate_val*4)/4), 100);
    return true;
}